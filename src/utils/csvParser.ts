  ...
  const players = result.data
    .map((row): RankedPlayer | null => {
      ...
      return {
        ...
        team: col(row, 'team', 'nfl_team', 'current_team').toUpperCase(),
        sleeperId: null,
        isDrafted: false,
        pickNumber: null,
        round: null,
        draftedBy: null,
        age: parseNumber(col(row, 'age', 'current_age', 'player_age', 'player age', 'age_years', 'age years')),
        yearsExp: parseNumber(
          col(row, 'years_exp', 'years of experience', 'years exp', 'experience', 'yoe', 'exp'),
        ),
        ppg: parseNumber(col(row, 'ppg', 'talent_ppg', 'points_per_game', 'points per game', 'avg_points')),
